...
// Speciality images for product cards
...
const getProductImage = (index) => {
    return specialityImages[index % specialityImages.length];
};

function SpecialityPremix() {   
    ...
    const cities = ['all', 'Chennai', 'Madurai', 'Coimbatore', 'Salem', 'Tiruchirappalli', 'Tirunelveli'];
    ...
    useEffect(() => {
        if (selectedCity === 'all') {
            ...
            const filtered = products.filter(product => 
                product.name.toLowerCase().includes(selectedCity.toLowerCase()) ||
                product.description.toLowerCase().includes(selectedCity.toLowerCase())
            );
    ...
    if (loading) {
        return (
            <div className="min-vh-100 d-flex align-items-center justify-content-center" style={{ fontFamily: 'var(--font-body)', backgroundColor: 'var(--athangudi-cream)' }}>
                <p style={{ color: 'var(--chettinad-charcoal)' }}>Loading speciality products...</p>
            </div>
        );
    }
        ...
        <div className="min-vh-100 py-5" style={{ fontFamily: 'var(--font-body)', backgroundColor: 'var(--athangudi-cream)' }}>
            <div className="container">
                <div className="text-center mb-5">
                    ...
                    <h2 className="mb-2" style={{ fontFamily: 'var(--font-display)', color: 'var(--chettinad-maroon)', fontWeight: 600 }}>Speciality Premix Products</h2>
                    <p className="text-muted">Exclusive regional blends from Tamil Nadu</p>
                </div>
                ...
                <div className="row mb-4">
                    <div className="col-12">
                        <div className="chettinad-card p-4">
                            <h5 className="fw-bold mb-3" style={{ fontFamily: 'var(--font-display)', color: 'var(--chettinad-charcoal)' }}>Filter by City</h5>
                            <div className="d-flex flex-wrap gap-2">
                                {cities.map(city => (
                                    <button
                                        key={city}
                                        className={`btn ${selectedCity === city ? 'btn-chettinad-primary' : 'btn-chettinad-outline'}`}
                ...
                {filteredProducts.length === 0 ? (
                    <div className="text-center p-5 chettinad-card">
                        <p className="mb-0" style={{ color: 'var(--chettinad-charcoal)' }}>
                            {selectedCity === 'all' 
                                ? 'No speciality products available' 
                                : `No speciality products found in ${selectedCity}`}
                        </p>
                    </div>
                ) : (
                        ...
                        {filteredProducts.map((product, index) => (
                            <div key={product.id} className="col-md-4">
                                <div className="chettinad-card card h-100 border-0 overflow-hidden">
                                    <div style={{ borderTop: '4px solid var(--chettinad-ochre)' }}>
                                        <img 
...
export default SpecialityPremix;
